...
export default function NewROPage() {
    ...
    useEffect(() => {
        apiGet<DropdownOption[]>("/api/data/options?group=insurance_company", { cacheMs: 60_000 }).then(setInsuranceOptions).catch(() => setInsuranceOptions([]));
        apiGet<DropdownOption[]>("/api/data/options?group=model", { cacheMs: 60_000 }).then(setModelOptions).catch(() => setModelOptions([]));
        apiGet<Branch[]>("/api/data/branches", { cacheMs: 60_000 }).then(setBranches).catch(() => setBranches([]));
    }, []);
    ...
    useEffect(() => {
        ...
        void apiGet<DropdownOption[]>(url, { cacheMs: 60_000 })
            .then((opts) => {
