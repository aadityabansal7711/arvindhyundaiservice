...
const statusPillClass = (status: StatusSection) => {
  ...
  if (["Approval Received", "Ready for Pre-Invoice", "Delivered"].includes(status)) {
    return "bg-emerald-50 text-emerald-800 ring-emerald-200";
  }
  if (["Total Loss / Disputed", "Billed but Not Ready"].includes(status)) {
    return "bg-rose-50 text-rose-800 ring-rose-200";
  }
  if (["Dismantle", "Mechanical", "Cutting", "Denting", "Painting", "Fitting"].includes(status)) {
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {derived.filtered.map((job) => (
                        <tr key={job.id} className="group hover:bg-sky-50/35 transition-colors">
                          ...
                          <td className="px-5 py-3 text-sm text-slate-700">
                            {job.branch_name
                              ?? (job.branch_id ? (branchNameById.get(job.branch_id) ?? "—") : "—")}
                          </td>
                          <td className="px-5 py-3 text-sm text-slate-700">
                            {job.reg_no ?? "—"}
                          </td>
                          ...
                          <td className="px-5 py-3 text-sm text-slate-700">
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-100 text-slate-700 text-xs font-semibold hover:bg-slate-200 transition-colors"
                            >
                          ...
                          <td className="px-5 py-3 text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-white border border-slate-200 text-slate-700 hover:text-sky-700 hover:bg-sky-50 transition-colors"
                              >
                                View
                              </button>
                            </div>
            ...
            <div className="p-4 sm:p-6 space-y-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-sm text-slate-500">
                    Full movement history for this job
                  </div>
                </div>
                <button
                  type="button"
