            ...
            <div className="space-y-6">
                ...
                <div className="bg-white p-3 sm:p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-3 sm:gap-4 items-center">
                    <div className="relative flex-1 w-full">
                        ...
                        <input
                            placeholder="Search by claim no, insurer, RO..."
                            value={searchTerm}
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Claim No</th>
                                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">RO No</th>
                                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Insurance Company</th>
                                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Claim Date</th>
                                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">HAP/NHAP</th>
                                ...
                                {isLoading ? (
                                    ...
                                    ros.map((ro) => (
                                        <tr key={ro.id} className="hover:bg-slate-50/50">
                                            <td className="px-6 py-3 text-sm text-slate-800">{ro.insuranceClaim?.claimNo ?? "—"}</td>
                                            <td className="px-6 py-3 text-sm font-medium text-slate-900">{ro.roNo}</td>
                                            <td className="px-6 py-3 text-sm text-slate-700">{ro.insuranceClaim?.insuranceCompany ?? "—"}</td>
                                            ...
                                            <td className="px-6 py-3">
                                                ...
                                                >
                                                    {hasClaimFilled(ro) ? <Pencil className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
                                                    {hasClaimFilled(ro) ? "Edit" : "Add"}
                                                </button>
                                            </td>
                                        </tr>
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40" onClick={closeEdit}>
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Claim No</label>
                                <input
                                    type="text"
                                    value={form.claimNo}
                                    onChange={(e) => setForm((f) => ({ ...f, claimNo: e.target.value }))}
